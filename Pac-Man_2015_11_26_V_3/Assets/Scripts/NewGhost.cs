...
public class NewGhost : MonoBehaviour {
	...
	void Start () {
		...
		pacMan = GameObject.Find("AM_PacManKnight");
	}

	void OnCollisionEnter(Collision other) 
	// När Pacman plockat upp ett kors och kolliderar med ett spöke
	{
		if (PacManScript.eat == true && other.gameObject.tag == "Player") 
		{
			flee = false;
			Eaten();
	...
	//de har varsitt hörn som flyktposition.
	{
		...
		{
			//Spöke1
			ghost.SetDestination(new Vector3(100,100,100));
